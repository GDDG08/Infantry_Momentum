...
void CapComm_Update() {
  ...
  capcomm->cap_rest_energy = sensor->CapRestEnergy;
...
void CapComm_SendCapCommData() {
  ...
  /* up data struct data    */
    ...
    if (HAL_UART_GetState(Const_SuperCap_UART_HANDLER) & 0x01) return; // tx busy
...
uint8_t CapComm_VerifyCapCommData(uint8_t* buff, uint16_t rxdatalen) {
    ...
    if (buff[0] != Const_CapComm_FRAME_HEADER_SOF) return FAILED;
